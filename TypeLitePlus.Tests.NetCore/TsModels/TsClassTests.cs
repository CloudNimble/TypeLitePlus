...
namespace TypeLitePlus.Tests.NetCore.TsModels
{
    public class TsClassTests {
        ...
        public void WhenInitializedWithInnerClass_ModuleIsSetToNamespaceAndOuterClass() {
            var target = new TsClass(typeof(TypeLitePlus.Tests.NetCore.TestModels.Outer.Inner));

            Assert.NotNull(target.Module);
            Assert.Equal(typeof(TypeLitePlus.Tests.NetCore.TestModels.Outer.Inner).Namespace + ".Outer", target.Module.Name);
        }

        [Fact]
        public void WhenInitializedWithSecondLevelInnerClass_ModuleIsSetToNamespaceAndAllOuterClasses() {
            var target = new TsClass(typeof(TypeLitePlus.Tests.NetCore.TestModels.Outer.Inner.SecondLevelInner));

            Assert.NotNull(target.Module);
            Assert.Equal(typeof(TypeLitePlus.Tests.NetCore.TestModels.Outer.Inner).Namespace + ".Outer.Inner", target.Module.Name);
        }
